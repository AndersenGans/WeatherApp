...
namespace WeatherApp_OKopot.Services
{
    public class WeatherControl
    {
        public async Task<List<WeatherModel>> DailyWeather(string cityName)
        {
            ...
            HttpResponseMessage response = await client.GetAsync("http://api.openweathermap.org/data/2.5/forecast/daily?q=" + cityName + "&units=metric&cnt=1&APPID=a0e092f89087b55a736a66d0519de7cb");
        ...
        public async Task<List<WeatherModel>> ManyDaysWeather(string cityName, int count)
        {
            var client = new HttpClient();
            HttpResponseMessage response = await client.GetAsync("http://api.openweathermap.org/data/2.5/forecast/daily?q=" + cityName + "&units=metric&cnt="+count+"&APPID=a0e092f89087b55a736a66d0519de7cb");
            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadAsStringAsync();
                var rootObj = JsonConvert.DeserializeObject<RootObject>(result); ;
